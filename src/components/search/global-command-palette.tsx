...
type NavItem = {
  ...
  keywords: string[];
  badge?: string;
  roles?: Array<'caregiver' | 'nurse' | 'doctor'>;
};
...
const ALL_NAV_ITEMS: NavItem[] = [
  {
    ...
    keywords: ['dashboard', 'home', 'today', 'overview', 'patient', 'stats', 'alerts'],
  },
  {
    title: 'Bedside Care',
    category: 'Daily Care',
  ...
  },
  {
    ...
    href: '/medications',
    icon: ClipboardList,
    keywords: ['medicine', 'medication', 'drugs', 'beers criteria', 'prescriptions', 'pill', 'dose', 'polypharmacy'],
  },
  {
    title: 'Vitals',
    category: 'Daily Care',
...
export function GlobalCommandPalette({ isOpen, onClose }: GlobalCommandPaletteProps) {
  ...
  const activeRole = role === 'doctor' || role === 'professional' ? 'doctor' : role;
  ...
  const handleSearchKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === 'ArrowDown') {
      e.preventDefault();
      setActiveIndex((idx) => (idx + 1) % filteredNav.length);
    } else if (e.key === 'ArrowUp') {
      e.preventDefault();
      setActiveIndex((idx) => (idx - 1 + filteredNav.length) % filteredNav.length);
    } else if (e.key === 'Enter') {
      e.preventDefault();
      handleNavigate(filteredNav[activeIndex]?.href || filteredNav[0].href);
    }
  };
    ...
    <>
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="max-w-3xl max-h-[85vh] overflow-hidden p-0 gap-0 rounded-2xl border-border/60 shadow-2xl bg-background/95 backdrop-blur-xl flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-2.5 px-1">
                ...
                <span className="text-xs font-medium text-muted-foreground font-mono">5 guides</span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {filteredScenarios.map((scenario) => (
                  <button
                    ...
                    aria-label={`Open bedside help for ${scenario.title}`}
                    className={cn(
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-bold text-foreground truncate">{scenario.title}</p>
            ...
            <div>
              <div className="flex items-center justify-between mb-2.5 px-1">
                ...
                <span className="text-xs font-medium text-muted-foreground font-mono">Older adults</span>
              </div>
                ...
                {filteredBeers.slice(0, 3).map((drug, i) => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <Badge variant="outline" className="text-[10px] font-bold uppercase tracking-wider border-amber-500/40 text-amber-600 bg-amber-500/10">
                        {drug.category}
                      </Badge>
                    </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-2.5 px-1">
                <span className="text-[11px] font-bold uppercase tracking-wider text-primary flex items-center gap-1.5">
                  ...
                  Quick Links ({filteredNav.length})
                </span>
              </div>
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      index === activeIndex
                        ...
                        : 'border-border/40 hover:bg-primary/5 hover:border-primary/30'
                    )}
                  >
          ...
          {/* Footer Bar */}
          <div className="p-3 bg-muted/20 border-t border-border/40 flex items-center justify-between text-xs text-muted-foreground px-4 shrink-0">
